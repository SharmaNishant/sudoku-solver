...
/**
 * This Function takes a Standard string and a delimiter to break the string
 * into substrings
 */
std::vector<std::string> split(const std::string &s, char delim) {
  std::vector<std::string> elems;
...
/**
 * This function takes an int values [1-9] and prints it in color on terminal.
 * Colors are pre-mapped to int values
 * */
void colorPrint(int n) {
  switch (n) {
  case (1):
    cout << "\033[1;31m" << n << "\033[0m";
    break;
  case (2):
    cout << "\033[1;32m" << n << "\033[0m";
    break;
  case (3):
  ...
  case (4):
    ...
    break;
  case (5):
    cout << "\033[1;35m" << n << "\033[0m";
    break;
  case (6):
    cout << "\033[1;36m" << n << "\033[0m";
    break;
  case (7):
    cout << "\033[0;31m" << n << "\033[0m";
    break;
  case (8):
    cout << "\033[0;36m" << n << "\033[0m";
    break;
  case (9):
    cout << "\033[0;32m" << n << "\033[0m";
    break;
  default:
    cout << n;
  }
...
/**
 * This function will drop the temperature/probability value after some
 * iterations. Input for this function are all global
 */
void DropTemperature() {
  assert(temperatureDropIterations > 0 &&
...
/**
 * This function generates two random locations for swapping. (One Row and Two
 * column, as we are maintaining rows)
 */
void GenerateSwapLocations(int &row, int &colOne, int &colTwo, Location &first,
                           Location &second) {
  row = rand() % 9;
  colOne = rand() % 9;
  colTwo = rand() % 9;
...
/**
 * Hill Climbing Implementation (no bad move probability, just swap if better
 * fitness is found)
 */
void HillClimbing(Sudoku &sudoku, unsigned int seed) {
  ...
  while (sudoku.GetFitness() > 0 && iterationCount < maxIterations) {
    ...
    if ((iterationCount % 1000000) == 0) {
      cout << "\033[A\033[2K";
      cout << iterationCount / 1000000
           << " Million iterations passed. Current fitness: "
...
/**
 * Simulated Annealing Implementation (we use a probability to make a bad move
 * and the probability goes down with time)
 */
void SimulatedAnnealing(Sudoku &sudoku, unsigned int seed) {
  ...
  while (sudoku.GetFitness() > 0 && iterationCount < maxIterations) {
    ...
    if (newFitness costOperator sudoku.GetFitness()) {
      sudoku.Swap(first, second);
    } else if (exp(((float)sudoku.GetFitness() - newFitness) / temperature) -
                   randValFloat >
               0) {
...
/**
 * Hill Climbing Implementation with a fixed probability of making a bad move
 * over time
 */
...
/**
 * This Function Reads Arguments from a pre defined launch file. This will help
 * in saving recompilation time
 */
void ReadFromFile(string filename, string &fileName, SimulationType &simType,
  ...
  if (launchFile.is_open()) {
    ...
    assert(temperature >= 0 && temperature <= 1 &&
           "Temperature/Probability cannot be less than 0 or more than 1");
    /**********/
    ...
    assert(temperatureDropConstant < 1 && temperatureDropConstant >= 0 &&
           "Temperature drop constant range should be [0,1)");
    /**********/
    ...
    assert(temperatureDropIterations > 0 &&
           "Temperature cannot be dropped in less than 1 iteration");
  } else {
