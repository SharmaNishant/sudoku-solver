...
void Sudoku::PrintSudoku() {
  ...
  for (int i = 0; i < 9; ++i) {
    ...
    for (int j = 0; j < 9; ++j) {
      ...
      cout << " ";
      colorPrint(this->sudoku[i][j].value);
      cout << " ";
      if (((j + 1) % 3) == 0) {
        cout << "║";
      }
...
int Sudoku::GetFitness() { return this->fitness; }

/** Make a swap after making sure the new fitnes is '<' or '<=' based on the
 * settings */
...
/* Fills sudoku with random values while making sure all the rows get unique
 * values*/
bool Sudoku::fillSudoku() {
  ...
  for (int row = 0; row < 9; ++row) {
    ...
    for (int col = 0; col < 9; ++col) {
      ...
      //			cout<<"erased "<<
      //this->sudoku[row][col].value<<endl;
    }
...
/** Sets the values of the fitness array with the number of times a value
 * appears in the sub-square for each sub-square*/
void Sudoku::generateQuadrantFitness() {
  ...
  //			cout<< col << " - " << this->quadrant[row][ col ]
  //<<endl;
  //		}
  //	}
}
...
void Sudoku::updateColumnFitness(int col, int toRemove, int toAdd) {
  ...
  // sanity check
  if (this->column[col][toRemove] < 0)
    this->column[col][toRemove] = 0;
  if (this->column[col][toAdd] > 9)
    this->column[col][toAdd] = 9;
...
void Sudoku::updateQuadrantFitness(int quad, int toRemove, int toAdd) {
  ...
  // sanity check
  if (this->quadrant[quad][toRemove] < 0)
    this->quadrant[quad][toRemove] = 0;
  if (this->quadrant[quad][toAdd] > 9)
    this->quadrant[quad][toAdd] = 9;
