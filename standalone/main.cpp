...
/**
 * If no arguments are passed, try to search for launch file to run accordingly,
 * otherwise blank sudoku with just hill climbing otherwise one can pass '1' or
 * 'file' followed by filename of the launch file to run this solver (See
 * ReadMe) or one can pass '2' or 'args' followed by arguments in the predefined
 * order to run solver solver (See ReadMe)
 */
int main(int argCount, char **argValues) {
  ...
  case (SIMULATED_ANNEALING):
    SimulatedAnnealing(sudoku, seed);
    break;
  case (HILL_CLIMBING_PROBABILITY):
    HillClimbingProbability(sudoku, seed);
    break;
  case (HILL_CLIMBING):
  default:
    HillClimbing(sudoku, seed);
